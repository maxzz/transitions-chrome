...
export function toBezierHandles(easing: unknown): BezierHandles | undefined {
    ...
    const isEaseList = Array.isArray(easing) && easing.length >= 4 && easing.slice(0, 4).every((value) => typeof value === "number" && !Number.isNaN(value));
    if (isEaseList) {
    ...
    if (points.length !== 4 || points.some((value) => Number.isNaN(value))) {
        return undefined;
    }
...
export function getValueAnimation(animations: RecordedAnimations, animationName: string, elementName: string, valueId: string) {
...
export function getTimestampedKeyframes(valueAnimation: ValueAnimationRecord): TimestampedKeyframe[] {
    ...
    const { delay = 0, duration = defaultTransitionOptionsUi.duration } = valueAnimation.options;
    
    return orderedKeyframes.map(
        (keyframe) => ({ ...keyframe, time: delay + (duration ?? defaultTransitionOptionsUi.duration) * keyframe.offset })
    );
}

export function updateValueAnimation(valueAnimation: ValueAnimationRecord, timestampedKeyframes: TimestampedKeyframe[]) {
    ...
    for (const timestamped of timestampedKeyframes) {
        ...
        valueAnimation.keyframes[keyframe.id] = { ...keyframe, offset: Number.isNaN(offset) || offset === Infinity ? 0 : offset };
    }
}
