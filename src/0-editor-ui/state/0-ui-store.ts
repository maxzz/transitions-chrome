...
export const getHistory = (state: EditorStore) => ({ undo: state.undo, redo: state.redo, enableHistory: state.enableHistory });
            ...
            scrubTo: (time) => {
                ...
                if (selectedAnimationName && animations[selectedAnimationName]) {
                    set({
                        isRecording: false,
                        animations: produce(animations, (draft) => {
                            if (draft[selectedAnimationName]) {
            ...
            deleteKeyframe: ({ elementName, valueId, id }) => {
                ...
                set({
                    animations: produce(animations, (draft) => {
                        ...
                        updateValueAnimation(valueAnimation, getTimestampedKeyframes(valueAnimation).filter((keyframe) => keyframe.id !== id));
                ...
                const newState: Partial<EditorStore> = {
                    animations: produce(animations, (draft) => {
                        ...
                        timestampedKeyframes.splice(insertionIndex, 0, {
                            ...
                            easing: nearestKeyframe?.easing ?? defaultTransitionOptionsUi.easing,
                            offset: 0,
                            time: snapped,
                            isEdited: true,
                            isUserCreated: true,
