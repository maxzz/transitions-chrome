...
export function generateCSSAnimationCode({ elements }: AnimationMetadata) {
    ...
    for (const elementName in elements) {
        ...
        for (let i = 0; i < elementAnimation.length; i += 1) {
            ...
            const cssKeyframeDefinitions = sortKeyframesByOffset(keyframes).map(
                ({ offset, value }, index, array) => {
        ...
        if (valueTransforms.size) {
            elementStyles += generateCSSTransform(valueTransforms);
        }
    ...
    allKeyframes.forEach(
        (keyframes, name) => {
            ...
            for (const { offset, value, easing, name: propertyName } of keyframes) {
                ...
                if (easing) {
                    css += newLine(2, `animation-timing-function: ${easing};`);
                }
                css += newLine(1, `}`);
                css += newLine(0, ``);
            }
