...
export const isEasingGenerator = (easing: unknown): easing is { createAnimation: (...args: never[]) => unknown; } => typeof easing === "object" && Boolean(easing && "createAnimation" in easing);
export const isCubicBezier = (easing: unknown): easing is number[] => Array.isArray(easing) && isNumber(easing[0]);
export const isEasingList = (easing: unknown): easing is unknown[] => Array.isArray(easing) && !isNumber(easing[0]);

export const mix = (min: number, max: number, progressValue: number) => -progressValue * min + progressValue * max + min;
...
export function keyframesList<T>(keyframes: T | T[]) {
    return Array.isArray(keyframes) ? keyframes : [keyframes];
}

export function pipeToCamel(str: string) {
    return str.replace(/-([a-z])/g, (match) => match[1].toUpperCase());
}

export function getEasingPoints(easing: string) {
    return easing.replace("cubic-bezier(", "").replace(")", "").split(",").map(parseFloat);
}
...
export function sortKeyframesByOffset<T extends { offset: number; }>(keyframes: Record<string, T>) {
