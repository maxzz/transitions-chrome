...
flowchart TB
  subgraph InspectedTab["Inspected tab — not the extension"]
    subgraph MainWorld["MAIN world — page JS heap"]
      ...
      Client["Injected page client IIFE<br/>src/1-context-script/client.ts<br/>store, recording, plugins, inspect"]
  ...
  subgraph MainExtension["MAIN EXTENSION — DevTools UI, own pages"]
    ...
    Editor["Panel iframe<br/>src/editor/index.html<br/>→ src/0-editor-ui"]
...
| **Main extension** | Extension pages / DevTools | `devtools/index.ts` + the panel iframe. User-facing UI. Talks to the SW; injects into MAIN via `inspectedWindow.eval`. |
...
| Page client | `src/1-context-script/client.ts` → IIFE | MAIN | Must see real `getAnimations()` and set `__MOTION_DEV_TOOLS_RECORD`. No `chrome.*`. |
...
| Editor | `src/editor/*` + `src/0-editor-ui/*` | Panel iframe | Timeline UI. Talks to the SW on port `devtools-page`. Can `inspectedWindow.eval` into MAIN. |
