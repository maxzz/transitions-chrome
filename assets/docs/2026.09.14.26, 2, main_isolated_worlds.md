...
Site code cannot read isolated-world variables. Isolated code cannot read `window.__TM_MOTION_DEV_TOOLS` unless MAIN puts it there. The only cheap cross-heap channel on the tab is `window.postMessage` (plus the shared DOM).
...
flowchart TB
  subgraph Tab["One inspected tab / frame"]
    subgraph Main["MAIN world — page JS heap"]
      ...
      Flags["window.__TM_MOTION_DEV_TOOLS"]
      Site --> Client
      Client --> Flags
    end
...
| --- | --- | --- | --- |
| Context script — client | `src/1-context-script/0-all/0-client-entry.ts` → `page-client.js` | Runs **in MAIN** | Must see real CSS / Motion animations and set `__TM_MOTION_DEV_TOOLS_RECORD`. Must **not** call `chrome.*`. |
...
sequenceDiagram
  ...
  Client->>Client: plugins + __TM_MOTION_DEV_TOOLS
...
| Set `window.__TM_MOTION_DEV_TOOLS_RECORD` | MAIN | Motion One on the site reads that global. |
